...
- `status` (`PROJECTED` | `CONFIRMED` | `SKIPPED`, not null, default `'PROJECTED'`): Event lifecycle status.
...
- `status` (`DRAFT` | `CONFIRMED` | `ARCHIVED`, not null, default `'DRAFT'`): Plan status.
...
- `amount` (numeric(12,2), not null): Dollar amount allocated.
- `ruleApplied` (text, not null): Explainability reasoning generated by the waterfall engine.
...
- `poolId` (uuid, nullable): Allocated pool debited/credited.
...
- `transactionType` (text, nullable): Operational subtype (`EXPENSE`, `INCOME_SPLIT`, `TRANSFER_OUT`, `TRANSFER_IN`, etc.).
- `amount` (numeric(12,2), not null): Movement amount.
- `note` (text, nullable): User note or transfer description.
- `source` (text, not null, default `'MANUAL'`): Source of record (`MANUAL`, `CSV`, `SPLIT`).
- `recordedAt` (timestamp, not null, default `now()`): Transaction date in UTC.
- `idempotencyKey` (text, nullable, unique): Duplicate submission prevention key.
...
- `status` (`PENDING` | `PAID` | `SKIPPED`, not null, default `'PENDING'`): Payment lifecycle status.
- `paidDate` (date, nullable): Reconciliation timestamp.
...
- `status` (`PENDING` | `COMPLETED` | `SKIPPED`, not null, default `'PENDING'`): Lifecycle status.
