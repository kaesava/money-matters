...
export async function seedDatabase(connectionString: string, envLabel: string) {
  ...
  await db.insert(transactionLedger).values([
    { poolId: everydayPool.id, categoryId: groceriesCat.id, bankAccountId: primaryAccount.id, flowType: "CREDIT", transactionType: "INCOME_SPLIT", amount: "1200.00", idempotencyKey: "payday-alloc-everyday-2026-07-01", note: "Payday Allocation — Everyday Pool", source: "AUTO", recordedAt: new Date("2026-07-01T09:05:00Z"), tenantId, appId, createdBy: userId, updatedBy: userId },
    { poolId: billsPool.id, categoryId: mortgageCat.id, bankAccountId: primaryAccount.id, flowType: "CREDIT", transactionType: "INCOME_SPLIT", amount: "1859.00", idempotencyKey: "payday-alloc-bills-2026-07-01", note: "Payday Allocation — Bills Pool", source: "AUTO", recordedAt: new Date("2026-07-01T09:05:00Z"), tenantId, appId, createdBy: userId, updatedBy: userId },
    { poolId: emergencyPool.id, bankAccountId: savingsAccount.id, flowType: "CREDIT", transactionType: "OPENING_BALANCE", amount: "10000.00", idempotencyKey: "initial-goal-emergency-2026-07-01", note: "Initial Reserve — Emergency Goal", source: "MANUAL", recordedAt: new Date("2026-07-01T09:00:00Z"), tenantId, appId, createdBy: userId, updatedBy: userId },
    { poolId: kaesavaPrivatePool.id, bankAccountId: kaesavaPrivateBank.id, flowType: "CREDIT", transactionType: "INCOME_SPLIT", amount: "300.00", idempotencyKey: "payday-alloc-kaesava-2026-07-01", note: "Payday Allocation — Personal Pool", source: "AUTO", recordedAt: new Date("2026-07-01T09:05:00Z"), tenantId, appId, createdBy: userId, updatedBy: userId },
    { poolId: everydayPool.id, categoryId: groceriesCat.id, bankAccountId: primaryAccount.id, flowType: "DEBIT", transactionType: "EXPENSE", amount: "142.50", idempotencyKey: "expense-groceries-debit-2026-07-02", note: "Woolworths Supermarket", source: "MANUAL", recordedAt: new Date("2026-07-02T10:30:00Z"), tenantId, appId, createdBy: userId, updatedBy: userId },
    { poolId: everydayPool.id, categoryId: diningCat.id, bankAccountId: primaryAccount.id, flowType: "DEBIT", transactionType: "EXPENSE", amount: "48.00", idempotencyKey: "expense-dining-debit-2026-07-04", note: "Local Cafe Brunch", source: "MANUAL", recordedAt: new Date("2026-07-04T09:15:00Z"), tenantId, appId, createdBy: userId, updatedBy: userId },
  ]);
