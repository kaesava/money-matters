...
describe('BillLifecycleFSM Engine', () => {
  ...
  describe('Transition Validation Logic', () => {
    it('allows valid transitions from PENDING', () => {
      ...
      expect(canTransitionBillLifecycleStatus('PENDING', 'SKIPPED')).toBe(true);
    ...
    it('prevents transitions from terminal states CONFIRMED and SKIPPED', () => {
      ...
      expect(canTransitionBillLifecycleStatus('SKIPPED', 'PENDING')).toBe(false);
      expect(canTransitionBillLifecycleStatus('SKIPPED', 'CONFIRMED')).toBe(false);
    });
  ...
  describe('Design Token Mappings', () => {
    it('contains valid Serene Finance token definitions for every status', () => {
      const statuses = ['PENDING', 'RING_FENCED', 'CONFIRMED', 'OVERDUE', 'SKIPPED'] as const;
