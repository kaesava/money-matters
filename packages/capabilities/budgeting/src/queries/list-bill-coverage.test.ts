...
import { categories, transactionLedger, incomeEvents, expenseEvents } from "@money-matters/db";
...
describe("listBillCoverageQuery", () => {
  ...
  it("returns NO_SCHEDULE for bill categories with no upcoming events", async () => {
    const mockCategories = [
      { id: "cat-1", name: "Netflix", type: "REGULAR", monthlyAmount: "20.00" },
    ];
    const mockTxs: any[] = [];
    ...
    expect(result.items[0].categoryName).toBe("Netflix");
  ...
  it("returns COVERED when bills pool balance is sufficient for upcoming bills", async () => {
    ...
    const mockTxs = [
      { categoryId: "cat-1", amount: "200.00", flowType: "CREDIT" },
    ];
    const mockPaychecks = [
      { expectedDate: "2026-09-01", expectedAmount: "2000.00", status: "UPCOMING" },
    ];
    const mockUpcomingBills = [
      { categoryId: "cat-1", expectedDate: "2026-08-28", expectedAmount: "150.00", status: "UPCOMING" },
    ];
  ...
  it("returns SHORT_BY when bills pool balance is insufficient for upcoming bills", async () => {
    ...
    const mockTxs = [
      { categoryId: "cat-1", amount: "100.00", flowType: "CREDIT" },
    ];
    const mockPaychecks = [
      { expectedDate: "2026-09-01", expectedAmount: "2000.00", status: "UPCOMING" },
    ];
    const mockUpcomingBills = [
      { categoryId: "cat-1", expectedDate: "2026-08-28", expectedAmount: "500.00", status: "UPCOMING" },
    ];
