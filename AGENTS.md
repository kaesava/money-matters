...
- **Standardized Confirmation Modals**: All user action confirmations (e.g. Delete, Mark Paid, Private Account, Archive) MUST use the reusable `<ConfirmDialog />` component from `@money-matters/ui`. Native browser `confirm()` popups and ad-hoc window alerts are strictly banned.
...
- **Form Error Handling & Validation Parity**: Native browser HTML5 validation bubbles (`required` attribute) are strictly banned. All form validation MUST be driven by Zod schemas, with field errors rendered consistently as subtle red helper text directly below the corresponding input element. Primary submit buttons MUST remain disabled unless form state is dirty and valid (`!isDirty || !isValid`). Mandatory field labels MUST include a red asterisk (`*`). First editable inputs MUST receive `autoFocus`. Amount inputs MUST enforce non-negative values, max 12 digits, and max 2 decimal places.
- **Modal Component Consolidation & Discard Confirmation Parity**: Proactively merge separate Create and Edit modals for the same entity into single unified components using an `isEdit` flag. Closing a modal via Cancel button, Escape key, or backdrop click MUST trigger a Discard Changes confirmation dialog whenever form state is dirty (`isDirty`). `ModalDialog` handles this automatically when passed the `isDirty` prop.
...
- **Universal Table Skeleton Loading & Conditional Pagination**: Table loading states MUST use `<SkeletonTable />`. Pagination controls (`<PaginationBar />`) MUST be conditionally rendered under tables ONLY if total record count is 5 or more (`data.length >= 5`).
