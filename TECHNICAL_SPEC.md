...
| `@money-matters/capability-budgeting` | `/dashboard/categories` (Health/Type filters, sort, CRUD), `MoveMoneyModal`, `UpcomingExpenseModal`, `PaydayPreviewModal`, `deleteUpcomingEventCommand` | `/(app)/categories` (`MobileFilterBar`, sort, health stat chips, `CategoryFormModal` CRUD), `MoveMoneyModal`, `UpcomingExpenseModal`, `PaydayPreviewWizard`, `deleteUpcomingEventCommand` | 100% Tested & Documented |
| `@money-matters/capability-transactions` | Main Dashboard Quick Actions (Expense & Income tabs with Bill Name/Income Source Name, CanAfford), `/dashboard/transactions` (Search, Filters, Sort, CSV export) | `home.tsx` Quick Actions (`user_preferences` collapse sync, CanAfford), `/(app)/transactions` (`MobileFilterBar`, Sort, Native CSV Share, Category hyperlinks) | 100% Tested & Documented |
