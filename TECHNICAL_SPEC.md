...
3. **`DEFICIT REPAIR` (Step 3)**: Restores any overdrawn/negative pool balances (`currentBalance < 0`) to $0. Secured shelter and essential bills take precedence over discretionary overdraft repairs.
4. **`COMMITTED GOALS` (Step 4)**: Prioritizes committed target-date goals sorted by target date. When target date is on or before next payday, allocates 100% of remaining gap; otherwise paces gap across remaining paychecks.
5. **`EVERYDAY TIME-BASED ALLOWANCE` (Step 5)**: Allocates cycle allowance ($1/26$ fortnightly, $1/12$ monthly). Supports top-up to cap (`rolloverRule === 'RESET'`) and default full fresh deposit (`'ROLLOVER'` / `'SWEEP'`).
6. **`UNCOMMITTED GOALS & RESIDUAL SURPLUS SWEEP` (Step 6)**: Funds uncommitted goals and sweeps 100% of remaining residual cents into the designated surplus bucket (`isSurplusTarget === true`). Unallocated cash is strictly $0.00.
