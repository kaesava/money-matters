...
- **Unified Cumulative Waterfall Projection Engine (`runCumulativeProjection`)**: Centralized math utility (`packages/capabilities/budgeting/src/engine/cumulative-projection.ts`) consumed by both the Matrix Planning Grid (`computeMatrixProjection`) and the Income Split Drawer (`previewPaydayQuery`). Sequentially projects pool balances starting from current ledger balances across all pending income events (`status !== 'CONFIRMED'`) sorted chronologically (`expectedDate` ASC, then `id` ASC for same-day determinism).
- **Scheduled Expense Deduction Cascade**: Between each simulated income event $i$ and $i+1$, any pending scheduled expenses (`status === 'PENDING'`) due in that date interval are subtracted from running pool balances. As a result, subsequent income events $i+1$ calculate allocations against the reduced pool balance, automatically allocating additional funds to refill pools.
