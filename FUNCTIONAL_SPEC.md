...
- **Dedicated Income Split Hero Screen (`/dashboard/income-split`)**:
  - *Distraction-Free Focus Mode*: Parameterized route `/dashboard/income-split?id=[incomeEventId]&returnTo=[path]`. Hides sidebar and floating action button to maximize horizontal real estate. Direct navigation without an ID gracefully redirects to `/dashboard/income-and-bills`. Refresh-safe (`F5`) and deep-linkable.
  - *Sticky Command Panel*: Live Paycheck Amount & Date editor with prompt to "Re-run Waterfall with new amount" when paycheck figure changes; Reactive Surplus Meter dynamically displaying remaining safe-to-spend funds with live visual proportion bar; and 1-Tap Payday Transfer Checklist (`PaydayTransferCard`).
  - *Interactive Allocation Canvas*: Categorized sections for Everyday, Bills, and Goals. Goals and discretionary Everyday pools feature smooth Serene Finance sliders ($5 step size on drag, dual-bound to direct cent input, with milestone ticks for proposed and target amounts). Bills pools feature clean numeric inputs with quick-action funding chips (`[100% Target]`, `[Skip]`).
  - *Reactive Auto-Surplus & Elastic Deficit*: The designated Surplus pool automatically absorbs residual funds in real time without its own slider. If allocations exceed income, Surplus turns red with an active deficit banner (`-$X.XX Deficit`), and "Save" / "Run Income Split" buttons are disabled until balanced.
  - *Dual Restore Controls*: Top-bar global "Reset Edits" button alongside per-pool inline `⟲ Reset` chips to restore initial proposed figures. Saved plans provide separate "Reset Edits" (to saved baseline) and "Revert to Auto" (wipes custom plan, re-running the 5-step waterfall).
  - *Full State Parity*: Handles Draft/Pending paydays (interactive split studio) and Confirmed paydays (read-only receipt breakdown with interactive bank transfer checklist and "Unsave" capability).
