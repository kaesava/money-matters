...
- **Dedicated Income Split Hero Screen (`/dashboard/income-split`)**:
  ...
  - *Clean Serene Finance Pool Table (`IncomeSplitPoolTable`)*: High-density structured table replacing busy card stacks with 100% column header/cell alignment parity (Left for Pool & Account, Center for Target, Right for Balance, Right for Allocation & Controls). Features collapsible group headers (`▼ Everyday Pools`, `▼ Bills Pools`, `▼ Goals`) with group subtotals allowing users to collapse reviewed sections. Removes noisy per-pool auto tags. Bills rows feature quick chips `[100%]`, `[$0]` + precision input; Goals rows feature dual-bound slider tracks.
