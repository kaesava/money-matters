...
import { PaydayTransferCard, PaydayTransferLine } from "../../../../components/web/PaydayTransferCard";
...
export interface IncomeSplitCommandPanelProps {
  ...
  readonly transferLines: PaydayTransferLine[];
    ...
    <aside className="w-full lg:w-96 shrink-0 space-y-5">
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-xs space-y-4">
        <h2 className="text-xs font-black uppercase tracking-wider text-zinc-500">
          {t("paydayDrawer.reviewIncome", { defaultValue: "Review Income" })}
        </h2>
      ...
      {/* 2. Reactive Live Surplus Gauge Card */}
      <div className={`border rounded-2xl p-5 shadow-xs space-y-4 transition-colors ${
        ...
        {isDeficit && (
          <div className="p-3 bg-red-100 dark:bg-red-950/80 border border-red-300 dark:border-red-800 rounded-xl text-xs text-red-800 dark:text-red-200 font-bold space-y-1">
            <span>⚠️ {t("paydayDrawer.deficitAlert", { amount: fmt(Math.abs(sweepPoolRemainder)), defaultValue: `Over-allocated by ${fmt(Math.abs(sweepPoolRemainder))}. Reduce other pools to balance.` })}</span>
          </div>
        )}
        ...
        <div className="space-y-1.5 pt-1">
          <div className="h-2.5 w-full bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden flex">
            <div style={{ width: `${billsPercent}%` }} className="bg-blue-500 h-full" title={`Bills: ${billsPercent.toFixed(0)}%`} />
            <div style={{ width: `${goalsPercent}%` }} className="bg-indigo-500 h-full" title={`Goals: ${goalsPercent.toFixed(0)}%`} />
            <div style={{ width: `${surplusPercent}%` }} className="bg-emerald-500 h-full" title={`Surplus: ${surplusPercent.toFixed(0)}%`} />
          </div>
